...
- (void)layoutSubviews {
    ...
    if (_isPhotosTask) {
        ...
        _subArrowIcon.image = [[PWIcons arrowIconWithColor:[PWColors getColor:PWColorsTypeTintWebColor] size:CGSizeMake(arrowSize, arrowSize)] imageWithRenderingMode:UIImageRenderingModeAlwaysTemplate];
...
- (void)setTaskObject:(PDTaskObject *)taskObject {
    ...
    if (taskObject.type.integerValue == PDTaskObjectTypeWebAlbumToLocalAlbum) {
        ...
        NSString *id_str = taskObject.to_album_id_str;
        ...
        [taskObject.photos enumerateObjectsUsingBlock:^(PDLocalPhotoObject *obj, NSUInteger idx, BOOL *stop) {
            if (idx >= 3) {
                *stop = YES;
                return;
            }
